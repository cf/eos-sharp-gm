...
namespace EosSharp.Core.Helpers
{
    ...
    public class CryptoHelper
    {
        ...
        public static readonly int GM_SIGN_KEY_DATA_SIZE = 104;//105
        ...
        public static KeyPair GenerateKeyPair(string keyType = "sha256x2")
        {
            ...
            var curve = MultiSignProvider.GetX9ECParamsFromKeyType(keyType);
            ...
            var key = privateKey.D.ToByteArrayUnsigned();
            var pubKey = publicKey.Q.GetEncoded(true);
            return new KeyPair()
            {
                PrivateKey = KeyToString(key, keyType, keyType == "R1" ? "PVT_R1_" : (keyType == "GM" ? "PVT_GM_" : null)),
                PublicKey = KeyToString(pubKey, keyType != "sha256x2" ? keyType : null, keyType == "R1" ? "PUB_R1_" : (keyType == "GM" ? "PUB_GM_" : "EOS"))
            };
        ...
        public static byte[] SignStringToBytes(string sign)
        {
            ...
            if (sign.StartsWith("SIG_GM_"))
                return StringToKey(sign.Substring(7), GM_SIGN_KEY_DATA_SIZE, "GM");
        ...
        {
            var keyBytes = Base582.Decode(key);
            byte[] digest = null;
            int skipSize = 0;
            ...
            if(keyType == "sha256x2")
            {
                // skip version
                skipSize = 1;
                digest = Sha256Manager2.GetHash(Sha256Manager2.GetHash(keyBytes.Take(size + skipSize).ToArray()));
            }
            else if(!string.IsNullOrWhiteSpace(keyType))
            {
                ...
                digest = Ripemd160Manager2.GetHash(SerializationHelper.Combine(new List<byte[]>() {
            ...
            {
                digest = Ripemd160Manager2.GetHash(keyBytes.Take(size).ToArray());
            }
        ...
        public static string KeyToString(byte[] key, string keyType, string prefix = null)
        {
            ...
            if (keyType == "sha256x2")
            {
                digest = Sha256Manager2.GetHash(Sha256Manager2.GetHash(SerializationHelper.Combine(new List<byte[]>() {
                    new byte[] { 128 },
                    key
                })));
            }
            ...
            {
                digest = Ripemd160Manager2.GetHash(key);
            }

            if(keyType == "sha256x2")
            {
                return (prefix ?? "") + Base582.Encode(SerializationHelper.Combine(new List<byte[]>() {
                    new byte[] { 128 },
                    key,
